...
module.exports.createStudentDBService = async (studentDetails) => {
    try {
        ...
        const existingStudent = await studentModel.findOne({phone:studentDetails.phone});
        if(existingStudent){
            return resizeBy.status(400).json({error:"User with phone number already exists"})
        }
